...
Os dados coletados da severidade (Escala Davis) e da contagem de plantas atacaas não seguem uma distribuição normal, e sim a distribuição de Poisson, em que os valores da variável resposta são números inteiros maiores ou iguais a zero. Nesse tipo de distribuição, números zeros são muito frequentes e, portanto, tendenciam a curva de distribuição para a esquerda.

Para analisar esse tipo de dados, é preciso utilizar de uma função de modelos lineares generalizados para encontrar um modelo que melhor se adapte aos resultados, para assim, conseguirmos extrair informações sobre seu comportamento e comparar a interação existente entre as variáveis presentes.
